...
function misc.getAdminlist(chat_id)
	...
	for i,admin in pairs(list.result) do
		...
		if admin.status == 'administrator' then
			adminlist = '['..(admin.user.username or admin.user.first_name)..'](https://telegram.me/'..(admin.user.username or '')..')'
			count = count + 1
		elseif admin.status == 'creator' then
			creator = '['..(admin.user.username or admin.user.first_name)..'](https://telegram.me/'..(admin.user.username or '')..')'
			end
		end
