...
plugins = {} -- Load plugins.
	...
	if on_reload then
		...
		api.sendAdmin('*BOT RUNNING!*', true)
...
on_msg_receive = function(msg) -- The fn run whenever a message is received.
	...
	if not msg then
		...
		if msg.date < os.time() - 30 then return end -- Do not process old messages.
		...
		for i, plugin in pairs(plugins) do
			...
			if not continue then return end
						...
						if msg and not msg.cb then
 						 if msg.chat.type == 'private' then
 						  db:incr('ttlbotmsgs:pv')
  						  elseif msg.chat.type ~= 'private' then
  						 db:incr('ttlbotmsgs:gp')
  						 end
  						 elseif msg.cb then
  						 if msg.chat.type == 'private' then
  						 db:incr('ttlbotinlines:pv')
  						  elseif msg.chat.type ~= 'private' then
 						   db:incr('ttlbotinlines:gp')
   						 end
  						 end
		for i,plugin in pairs(plugins) do
